...
- Projection is idempotent for the event currently in flight. HEU-576 added this. Redelivering an event whose projection is still current completes it rather than failing. A redelivery that is no longer current is refused rather than reapplied. That covers a placement whose node was since removed. It also covers one whose user a later event re-placed. Redelivery age does not change either case. Two residuals remain. A removal whose store write never landed still fails on redelivery. See HEU-777. A removal redelivered after its user was placed again removes the wrong node. See HEU-789.
