...
**Summary:** The Go/Rust subprocess protocol uses NDJSON (one JSON object per line) over stdin/stdout. Requests carry an `id` and `op` field. Responses carry the same `id` with an `ok` boolean. A fixed error code taxonomy (`STRUCTURE_NOT_FOUND`, `USER_NOT_FOUND`, `INVALID_PARAMS`, etc.) enables typed error handling on the Go side. The Rust worker recovers from panics via `catch_unwind`. Go supports context cancellation for blocked reads. JSON object key order is explicitly not part of the contract; Go decodes by struct tag and must not depend on it (HEU-648). It is also not stable: since HEU-743 removed the `serde_json::Value` hop, `HashMap`-backed fields emit in an order fixed per map instance. Snapshotting the same live structure twice is reproducible. A rebuilt or restored copy of it is not.
...
- Store projection happens before engine projection. The event exists for replay whatever the engine does. A refused `add_root` cannot leave the table in a state the loader rejects. Migration 000006 refuses a second active depth-0 row. The insert fails before the engine is called (HEU-810). `handleRootAdded` still compensates its own row when the engine refuses after a successful insert. It reports what the store holds when the caller's context was cancelled first.
...
- Projection is idempotent for the event currently in flight. HEU-576 added this. A redelivered placement whose projection is still current completes rather than failing, unless its parent's row was since removed. A placement is no longer current once its node was removed, or once a later event re-placed the user, and is refused rather than reapplied. Redelivery age does not change that. A redelivered removal that the engine refuses converges when its own tombstone is in the store. That holds even when a later placement of the user never reached the engine. One arriving after a later placement landed in full can still remove the wrong node (HEU-789). One whose store write never landed fails with a typed error. The remaining cases where a redelivery is refused or unsafe are tracked separately.
- Decided on HEU-820, built under HEU-811. A tombstoned row carries the ID of the event that removed it, in `removed_by_event_id` (migration 000007). The column is null while the row is active. A removal's reconcile compares its own event ID against `removed_by_event_id`, not against `tree_nodes.id`. `tree_nodes.id` holds the placing event's ID, so the two can never match. The stamp says which event removed a row. It does not order several removals of one user. HEU-789 asks that question. When one event has stamped more than one row, `GetNodeByRemovalEvent` returns the newest tombstone.
- The column has no backfill. No tombstone predates it, because no production consumer exists yet. Migration 000007 must run before HEU-301 wires one.
- `DeleteNodeAndResponsor` fails when its soft delete matches no active row. It writes nothing. Its error states what the store observed: how many active rows matched, and how many re-sponsor writes were not applied. It names no cause, because the store cannot see why the row was missing.
...
- The mechanism above is not limited to the toolchain bin directory. A hand-installed binary in a personal `bin` directory also precedes mise's shims. The analysis tools this repo pins are commonly installed that way. The ADR named the mechanism and one location. A different location is where the lint outage came from, recorded in HEU-806. The bullet below is how to find out which one applies to you.
- Setting `GOBIN` does not always hold. On 2026-09-18 an install run through `mise exec` discarded an explicit `GOBIN` and landed the binary inside mise's own install tree. `mise ls` reported the version mise had installed. `mise which` returned the binary that would actually run. The two disagreed. Nothing said so. Ask `command -v`. Run that path.
