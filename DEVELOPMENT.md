...
- Store projection happens before engine projection, so the event exists for replay whatever the engine does. A refused `add_root` cannot leave the table in a state the loader rejects: migration 000006 refuses a second active depth-0 row, so the insert fails before the engine is called (HEU-810). `handleRootAdded` still compensates its own row when the engine refuses after a successful insert, and reports what the store holds when the caller's context was cancelled first.
