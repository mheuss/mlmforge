...
- The mechanism above is not limited to the toolchain bin directory. A hand-installed binary in `~/.local/bin` also precedes mise's shims, and that is where this project's `golangci-lint` and `gitleaks` live. The ADR named the mechanism and one location. The other location is where the four-day lint outage came from.
