...
- Placement projection is idempotent for the event currently in flight. HEU-576 added this. Redelivering a placement whose projection is still current completes it rather than failing. One that is no longer current is refused rather than reapplied. That covers a placement whose node was since removed. It also covers one whose user a later event re-placed. Redelivery age does not change either case. Removal redelivery is not idempotent. A removal carries no reference to the placement it removed, so the store cannot tell one delivery from another. Its open cases are tracked separately.
