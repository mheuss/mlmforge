...
- Swapping `%w` for `%s` when a message moves into a typed error is safe only while no reachable cause implements `fmt.Formatter`. The two verbs render differently for one that does, because `%w` rewrites the verb to `v` before the formatter is consulted. Checked across this repo and the pinned pgx on 2026-09-16: no such error exists on the tree loader's paths. A future wrapper that introduces one breaks the loader's golden message test with a text change that looks unrelated to the wrapper that caused it.
