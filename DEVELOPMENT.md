...
- Swapping `%w` for `%s` when a message moves into a typed error is safe only while no reachable cause implements `fmt.Formatter`. The two verbs render differently for one that does, because `%w` rewrites the verb to `v` before the formatter is consulted. Checked on 2026-09-16 across this repo's own types and the pinned pgx, which found none. That is the scope checked, not the whole dependency graph. A future wrapper that introduces one breaks the loader's golden message test with a text change that looks unrelated to the wrapper that caused it.
