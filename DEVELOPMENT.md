...
- Projection is idempotent for the event currently in flight. HEU-576 added this. A redelivered placement whose projection is still current completes rather than failing, unless its parent's row was since removed. A placement is no longer current once its node was removed, or once a later event re-placed the user, and is refused rather than reapplied. Redelivery age does not change that. A redelivered removal converges when it fully projected and nothing has changed the user's placement since. One whose store write never landed fails with a typed error. The remaining cases where a redelivery is refused or unsafe are tracked separately.
