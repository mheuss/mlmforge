...
- Projection is idempotent for the event currently in flight. HEU-576 added this. A redelivered placement whose projection is still current completes rather than failing, unless its parent's row was since removed. A placement is no longer current once its node was removed, or once a later event re-placed the user, and is refused rather than reapplied. Redelivery age does not change that. A redelivered removal converges when its own tombstone is in the store, whatever has happened to the user's placement since. One whose store write never landed fails with a typed error. The remaining cases where a redelivery is refused or unsafe are tracked separately.
- A tombstoned row carries the ID of the event that removed it, in `removed_by_event_id` (migration 000007). The column is null while the row is active. A removal's reconcile compares its own event ID against this column, not against `tree_nodes.id`. That column holds the placing event's ID, so the two can never match. The stamp says which event removed a row. It does not order several removals of one user, which is the question HEU-789 asks. When one event has stamped more than one row, the read takes the newest tombstone. Decided on HEU-820, built under HEU-811.
- The column has no backfill. That relies on no tombstone predating it, which holds until HEU-301 wires a production consumer.
- `DeleteNodeAndResponsor` fails when its soft delete matches no active row, and writes nothing. Its error states what the store observed: how many active rows matched, and how many re-sponsor writes were not applied. It names no cause, because the store cannot see why the row was missing.
