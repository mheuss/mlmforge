...
**Validation happens once, at the boundary.** `handle_load_plan` is the only door config walks through. Everything in `WorkerState` is valid by construction. A handler does not re-validate and does not need to know how validation works.
