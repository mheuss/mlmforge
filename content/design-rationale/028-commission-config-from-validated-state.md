...
"Valid" means whatever the validators check, no more. Making them the sole point of trust raises the cost of a hole in one. HEU-612 was the worked example. `StreamlineCommissionConfig::validate` left `commissionable_depth` unbounded above and did not check `dynamic_compression` for ordering, gaps, or duplicates, so a hand-rolled plan could defeat the depth limit and pay one level's rate against another level's rank threshold. Those two holes are closed. The tree-level config doors below are still open. Concentrating trust at the boundary is still the right shape — it just means the boundary has to be worth trusting.
