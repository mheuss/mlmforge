...
- **Width manifest** (`engine/testdata/config_contract/width_manifest.json`): a single source of truth listing each width-constrained field by JSON pointer and its expected `uintN` width. `width_contract_test.go` drives per-field boundary assertions from it, and the manifest doubles as the human-readable index of the whole contract surface.
  ...
  **The manifest's `reason` field is unparsed, and a wrong one keeps a field unguarded.** `width_contract_test.go` matches entries by `go_struct` and `go_field` and never reads `reason`, so an `allow_list` exemption carries a free-text justification no test can falsify. HEU-723 found `RankDefinition.Ordinal` exempted as "Rust u32; not a narrow byte-width mirror" when the Rust field is `u16`. The wrong type is what kept it off the guarded list: a plan with `ordinal: 70000` clears the schema, clears Go's `int`, and dies at Rust serde, which is the opaque cross-boundary failure this contract exists to prevent (HEU-753). When you change a field's valid range, re-derive its manifest entry. An exemption outlives its justification silently.
...
- **Golden fixture guard** (`TestConfigContractFixturesMatchPipeline`, `genfixtures_test.go`): byte-compares each committed `engine/testdata/config_contract/fixtures/*.json` against live `translateToEngine` output, locking the wire shape and key order (`taggedStructure` emits `type` before `config` in each structure; a `map[string]any` would emit them alphabetically. Since HEU-648 both orders deserialize, so this is about keeping the golden bytes stable rather than about parsing. See the comment on it in `translate.go`). The Rust side (`config_width_contract.rs`) loads those same fixtures and asserts they deserialize and reject over-max values, so a `translate.go` change that desyncs the fixtures fails on the Go side before the Rust side reads a stale file. Regenerate intentionally with `REGEN_FIXTURES=1 go test ./internal/config/ -run TestGenerateConfigContractFixtures` (the guard requires the value to be exactly `1`).

- **Rate-map key-width guard**: level-keyed rate maps are Go `map[string]float64` but Rust `BTreeMap<u8, f64>`, so the *key* carries a byte-width too. `validateRateKeyWidths` / `validateU8MapKeys` (`rules.go`) reject a key outside `[0, 255]` or non-numeric, and the schema mirrors the bound with a `propertyNames` pattern. They also reject two spellings of one key in the same map (`duplicate_value`): the pattern is `^0*(...)`, so `"1"` and `"01"` are both valid property names that name one Rust `u8` entry, and one rate would silently win. The engine rejects the same collision (HEU-648).

**Streamline level keys do not go through this guard.** `validateU8MapKeys` takes a `map[string]float64` and is reached only via `validateRateKeyWidths` → `getRateTable`, whose switch covers Unilevel, Matrix, Stairstep and Generation and returns `nil` for `StreamlineCommission`. So a `"1"`/`"01"` collision on `dynamic_compression` never produces `duplicate_value`. It is still rejected, but by `non_sequential_levels` in `validateStreamlineCommission`: the sorted level list becomes `[1, 1, ...]` and fails the `n != j+1` check at index 1. The practical difference is the error the plan author sees — a sequencing complaint, not a duplicate one. Streamline's 1-based constraint (level 0 rejected) likewise comes from the schema's `propertyNames` pattern and `sortStreamlineLevels`, not from this guard. Verified during HEU-612. The AST scan above also recurses into map *keys*, so a narrow-key map typed with a signed key is caught too.
...
- A request-scoped worker handler that deserializes its own plan or structure config from request params bypasses even `CompensationPlan::validate`. The HEU-517 gate runs only in `handle_load_plan`. `handle_calculate_streamline` did this and was fixed by HEU-583; `handle_board_calculate_commissions` by HEU-603. Both now resolve their structure from the plan in `WorkerState`. The *create* door is still open: `handle_create_board_plan` (`engine/network-engine-worker/src/handlers/board_plan.rs`) deserializes a `BoardPlanConfig` straight from request params and stores it in `WorkerState.trees` without ever running `BoardPlanConfig::validate`. It is latent, because the stored copy's `cycle_commission` feeds no payout now that the calculate path rates from the plan, but HEU-607 tracks it. Treat request-scoped params as unvalidated input when adding or changing handlers, and prefer sourcing config from `WorkerState` (design rationale 028).
