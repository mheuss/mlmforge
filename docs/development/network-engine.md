...
- Placement redelivery is bounded (HEU-576). The scope is the event in flight. That scope is a constraint on HEU-301. Within it, a redelivered placement whose projection is still current completes. One that is no longer current is refused rather than reapplied, however old it is. A placement stops being current when its node was since removed, or when a later event re-placed the user. Removal redelivery is not idempotent. The row carries the placing event's id, never the removing one, so nothing in the store separates two deliveries of the same removal. Its open cases are tracked separately.
