...
- The agreement claim covers placement only. No tree event path calls `RemoveMatrixNode`, so a matrix removal cannot project (HEU-582). `TreeWriter.Remove` refuses a removal from a matrix tree before it appends anything. A matrix `node_removed` appended without the writer reaches `handleNodeRemoved`. That handler calls `RemoveNode` with no pruning mode, and the worker refuses it with `MISSING_PARAM`. The handler retries, writes nothing to the store, and returns the error.
