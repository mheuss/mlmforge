...
for _, tree := range trees {
	...
	switch {
	case err == nil:
		// Loaded, or there were no rows to load.

	case errors.As(err, &rejected):
		// The engine was never called. Other trees are unaffected.
		...
		// A bad config is a wiring bug. A failed read is infrastructure.
...
**Added:** Unreleased (HEU-576), removal stamp read in HEU-811
**Files:** `internal/networkengine/tree_consumer.go` (`withRetry`, `reconcileOutcome`, `reconcileNotApplicable`, `reconcileConverged`, `reconcileDiverged`, `reconcileInconclusive`), `internal/networkengine/tree_store.go` (`GetNodeByRemovalEvent`)
...
err := c.withRetry(ctx, op, treeID, userID, func() error {
    ...
    // The removal path asks first for the row its own event stamped. A read
    // keyed on tree and user prefers a later active placement over it.
    stamped, serr := c.store.GetNodeByRemovalEvent(ctx, treeID, eventID)
    if serr != nil {
...
**Solution:** `TreeWriter` holds a per-tree Postgres advisory lock from before the load until projection returns, on a connection of its own. Under the lock it rebuilds the tree in a scratch engine, redelivers the stream's last event, asks `check_mutation` whether the mutation would succeed, appends, and projects the stored copy. `MemoryTreeLocker` is a real per-tree lock for tests.
