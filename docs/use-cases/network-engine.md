...
**Solution:** Three layers. A gate at the top of the handler rejects everything checkable from the payload alone (stream identity, known `tree_type`, per-type position rules) before either projection, so a rejected event leaves no trace outside the EventStore. A partial unique index (`idx_tree_nodes_tree_parent_position_active`, migration 000004) arbitrates what the gate cannot see: two events claiming one slot resolve at the insert, loudly, with the store still reloadable. The insert names the primary key as its `ON CONFLICT` arbiter and does nothing on a match, so a row already carrying this event's id is skipped and reported rather than raising, while a conflict on any other constraint still raises. `BulkInsert` shares that statement inside one transaction, where a raised primary-key violation would abort the whole batch. The layers do not make the two projections atomic. The store insert still lands before the engine call, so an engine failure leaves a stored row the engine has not applied; since HEU-576 a redelivery of that event completes the engine side rather than failing at the insert.

**Notes:** Distinct from UC-NET-012, which preflights an irreversible bulk replay. Here each event is individually recoverable, so the gate stays thin (payload-checkable rules only) and the database owns cross-event races. The gate cannot check the matrix width bound because nothing persists width (HEU-554). The u8 ceiling is gated; the width..255 band is the documented residual. Redelivering an event whose projection is still current completes it (HEU-576); one that is no longer current, because the node was since removed or a later event re-placed the user, is refused rather than reapplied. That scope is the event in flight, which is a constraint on HEU-301, and arbitrary historical replay is HEU-789. `MemoryTreeStore` mirrors all three constraints and, like Postgres, separates a primary-key conflict from every other conflict, which is the distinction the discriminator rests on and is what makes it unit-testable against the double. The two stores do not agree on which constraint they name when several are violated at once (HEU-794).
