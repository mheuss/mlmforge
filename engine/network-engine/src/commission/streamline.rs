...
mod tests {
    ...
    /// The dynamic-compression twin of the unilevel forfeit test. This is the
    /// path HEU-608 is named for, so it gets its own pin: an unranked node
    /// must not compress out of a level threshold.
    #[test]
    fn an_empty_rank_does_not_clear_a_level_threshold() {
        let engine = make_engine(3);
        let structure = make_structure(
            vec![level(1, "associate", 0.10), level(2, "associate", 0.02)],
            5,
        );
        let mut plan = test_helpers::build_test_plan(
            test_helpers::default_eligibility(),
        ...
        // Ordinals are 1-based. HEU-723.
        plan.ranks = vec![rank_def("associate", 1)];

        // Chain 1 -> 2 -> 3. Volume at 3 walks up 2 then 1.
        let mut snapshots = HashMap::new();
        for (i, rank) in ["associate", "", "associate"].iter().enumerate() {
            snapshots.insert(
                test_uuid((i + 1) as u8),
                DistributorSnapshot {
                    rank: rank.to_string(),
        ...
        assert_eq!(
            root.level, 2,
            "the unranked node did not consume its level: {:?}",
            result.earnings
        );
    }
