...
mod tests {
    ...
    /// An unranked node must not compress out. Its rank is caller-asserted and
    /// resolves below every ordinal a plan can define, so compressing it would
    /// let a caller promote the upline a level by sending one fewer character
    /// than any real rank. It forfeits instead: earns nothing, consumes its
    /// level, and appears in the walk record.
    #[test]
    fn an_empty_rank_forfeits_rather_than_compressing_out() {
        let mut tree = UnilevelTree::new();
        tree.add_root(test_uuid(1), 0).unwrap();
        tree.add_node(test_uuid(2), test_uuid(1), test_uuid(1), 0)
            .unwrap();
        ...
        let mut structure = test_structure(test_rate_table());
        // Threshold at the lowest rank: every rank the plan defines clears it,
        // so an empty rank is the only accepted value that falls below.
        structure.compression = Some(crate::config::commission::CompressionConfig {
            enabled: true,
            mode: crate::config::commission::CompressionMode::SkipBelowRank,
        ...
        .expect("an empty rank is accepted");

        assert!(
            !result.earnings.iter().any(|e| e.earner_id == test_uuid(2)),
            "the unranked node earned: {:?}",
            result.earnings
        );
        let root = result
            .earnings
            ...
            .expect("the root should still earn");
        assert_eq!(root.level, 2, "the unranked node did not consume its level");
        assert!(
            result.walks[0]
                .steps
                .iter()
                .any(|s| s.node_id == test_uuid(2)),
            "the unranked node left no trace in the walk record"
        );
    }
