...
impl BinaryTree {
    ...
    pub fn validate_restored(&self) -> Result<(), SnapshotConsistencyError> {
        ...
        for (parent, children) in &self.slots {
            self.arena.check_slot_index(parent.0)?;
            for child in children.iter().flatten() {
                self.arena.check_slot_index(child.0)?;
            }
        }
        // A live node with no entry passes every check above and then panics
        // on the next query rather than returning a wrong answer.
        self.arena
            .check_every_live_node_has_a_slot(|slot| self.slots.contains_key(&slot))?;
        Ok(())
    }
...
mod tests {
    ...
    fn validate_restored_surfaces_an_arena_fault() {
        // Proves the arena delegation is present. Without this, deleting
        // self.arena.validate_restored() from validate_restored leaves every
    ...
    fn validate_restored_rejects_a_live_node_with_no_slots_entry() {
        ...
        // than a wrong answer: this file holds seven expect() calls that
        // assume the entry is present.
        let (mut tree, child) = binary_pair();
        tree.slots.remove(&child);
