...
//!
//! Builds the deep-sparse case the ticket names and reports serialized bytes
//! and peak RSS for one generation SameRank calculation. Runs only under the
//! `spike_heu556_volume` feature, which instruments the walk loops to record
//! non-consuming steps.
...
#[test]
fn deep_sparse_generation_same_rank_volume() {
    ...
    // Keep both alive past the high-water read.
    ...
    println!("--- HEU-556 deep-sparse generation SameRank ---");
