...
//!
//! Same fixture, third shape: traversed paths interned once and referenced by
//! walks, with stated decisions kept only for consuming steps. Separate test
//! binary so the peak RSS high-water mark is not inherited from another run.
...
#[test]
fn deep_sparse_generation_same_rank_interned_paths() {
    ...
    // Every walk's traversed node list, interned. Walks from the same region
    // of the tree share a path, so the sequence is stored once.
    #[derive(Serialize)]
    struct Decision {
        at: u32,
        outcome: &'static str,
        #[serde(skip_serializing_if = "Option::is_none")]
        earner_rank: Option<String>,
    }
    ...
    struct InternedWalk {
        ...
        stop: &'static str,
        #[serde(skip_serializing_if = "Option::is_none")]
        stopped_at: Option<Uuid>,
    }

    #[derive(Serialize)]
    struct InternedResult<'a> {
        earnings: &'a [network_engine::commission::types::CommissionEarning],
        paths: Vec<&'a [Uuid]>,
        walks: Vec<InternedWalk>,
        plan: &'a network_engine::commission::PlanIdentity,
    }

    let mut path_ids: Vec<Vec<Uuid>> = Vec::new();
    let mut path_index: HashMap<Vec<Uuid>, u32> = HashMap::new();
    let mut interned: Vec<InternedWalk> = Vec::with_capacity(result.walks.len());

    for w in &result.walks {
        let nodes: Vec<Uuid> = w.steps.iter().map(|s| s.node_id).collect();
        let path = *path_index.entry(nodes.clone()).or_insert_with(|| {
            path_ids.push(nodes);
            (path_ids.len() - 1) as u32
        });

        // Only consuming steps carry a stated decision. A non-consuming skip
        // is recomputable from the path, the snapshots and the plan.
        let decisions = w
            .steps
            .iter()
            .enumerate()
            ...
            .map(|(at, s)| Decision {
                at: at as u32,
                outcome: "paid",
                earner_rank: s.earner_rank.clone(),
            })
            .collect();

        interned.push(InternedWalk {
            index: w.index,
    ...
    let json = serde_json::to_string(&reduced).expect("to_string");
    ...
    println!("--- HEU-556 deep-sparse, interned paths + consuming decisions ---");
