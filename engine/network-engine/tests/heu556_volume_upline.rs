...
//!
//! Same fixture, reduced to the option this ticket's title offers as the
//! alternative: the traversed upline as a bare node sequence, with the
//! classifications dropped. Separate test binary so the peak RSS high-water
//! mark is not inherited from another run.
...
#[test]
fn deep_sparse_generation_same_rank_upline_only() {
    ...
    // The alternative shape: every step collapses to its node id. The walk
    // keeps the context a reader needs to recompute, and nothing else.
    #[derive(Serialize)]
    struct UplineWalk {
        index: u32,
        source_id: Uuid,
    ...
    let upline_walks: Vec<UplineWalk> = result
        ...
        .map(|w| UplineWalk {
            ...
            kind: "generation",
            rank: w.rank.clone(),
            nodes: w.steps.iter().map(|s| s.node_id).collect(),
            stop: "root_reached",
            stopped_at: w.stopped_at,
        })
        .collect();

    let reduced = UplineResult {
    ...
    println!("--- HEU-556 deep-sparse, upline sequence only ---");
